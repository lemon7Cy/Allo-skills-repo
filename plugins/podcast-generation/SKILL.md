...
Use `$SKILL_DIR/scripts/generate.py` for the bundled generator. Use Agent-provided workspace and output paths for user files; create output directories if needed.
...
python "$SKILL_DIR/scripts/generate.py" \
  ...
  --output-file $OUTPUT_DIR/generated-podcast.mp3 \
  --transcript-file $OUTPUT_DIR/generated-podcast-transcript.md
```
...
python "$SKILL_DIR/scripts/generate.py" \
  ...
  --output-file $OUTPUT_DIR/ai-history-podcast.mp3 \
  --transcript-file $OUTPUT_DIR/ai-history-transcript.md
```
...
- Podcasts and transcripts are saved in `$OUTPUT_DIR/`
