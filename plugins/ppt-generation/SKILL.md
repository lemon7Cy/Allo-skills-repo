...
Use `$SKILL_DIR/scripts/generate.py` for this skill's PPTX composer. For slide image generation, resolve the installed `image-generation` skill separately and derive `IMAGE_SKILL_DIR` from its `SKILL.md` path; then call `$IMAGE_SKILL_DIR/scripts/generate.py`. Use Agent-provided workspace and output paths for user files; create output directories if needed.
...
1. Read the image-generation skill: the installed `image-generation` skill `SKILL.md`
...
python "$IMAGE_SKILL_DIR/scripts/generate.py" \
  ...
  --output-file $OUTPUT_DIR/slide-01.jpg \
  --aspect-ratio 16:9
```
...
python "$IMAGE_SKILL_DIR/scripts/generate.py" \
  ...
  --reference-images $OUTPUT_DIR/slide-01.jpg \
  --output-file $OUTPUT_DIR/slide-02.jpg \
  --aspect-ratio 16:9
```
...
python "$IMAGE_SKILL_DIR/scripts/generate.py" \
  ...
  --reference-images $OUTPUT_DIR/slide-02.jpg \
  --output-file $OUTPUT_DIR/slide-03.jpg \
  --aspect-ratio 16:9

# Slide 4 references slide 3
python "$IMAGE_SKILL_DIR/scripts/generate.py" \
  --prompt-file $WORKSPACE_DIR/slide-04-prompt.json \
  --reference-images $OUTPUT_DIR/slide-03.jpg \
  --output-file $OUTPUT_DIR/slide-04.jpg \
  --aspect-ratio 16:9
```
...
python "$SKILL_DIR/scripts/generate.py" \
  ...
  --slide-images $OUTPUT_DIR/slide-01.jpg $OUTPUT_DIR/slide-02.jpg $OUTPUT_DIR/slide-03.jpg \
  --output-file $OUTPUT_DIR/presentation.pptx
```
...
Read the installed `image-generation` skill `SKILL.md` to understand how to generate images.
...
python "$IMAGE_SKILL_DIR/scripts/generate.py" \
  ...
  --output-file $OUTPUT_DIR/nova-slide-01.jpg \
  --aspect-ratio 16:9
```
...
python "$IMAGE_SKILL_DIR/scripts/generate.py" \
  ...
  --reference-images $OUTPUT_DIR/nova-slide-01.jpg \
  --output-file $OUTPUT_DIR/nova-slide-02.jpg \
  --aspect-ratio 16:9
```
...
python "$SKILL_DIR/scripts/generate.py" \
  ...
  --slide-images $OUTPUT_DIR/nova-slide-01.jpg $OUTPUT_DIR/nova-slide-02.jpg $OUTPUT_DIR/nova-slide-03.jpg $OUTPUT_DIR/nova-slide-04.jpg $OUTPUT_DIR/nova-slide-05.jpg \
  --output-file $OUTPUT_DIR/nova-presentation.pptx
```
...
- The PPTX file is saved in `$OUTPUT_DIR/`
