...
Use `$SKILL_DIR/scripts/analyze.py` for the bundled analyzer. Use Agent-provided upload, workspace, and output paths for user files; create output directories if needed.
...
- **File location**: Path(s) to uploaded Excel/CSV files under `$UPLOAD_DIR/`
...
python "$SKILL_DIR/scripts/analyze.py" \
  ...
  --output-file $OUTPUT_DIR/filtered-results.csv
```
...
```bash
python "$SKILL_DIR/scripts/analyze.py" \
  --files $UPLOAD_DIR/sales_2024.xlsx \
  --action inspect
```
...
python "$SKILL_DIR/scripts/analyze.py" \
  ...
  --output-file $OUTPUT_DIR/monthly-trends.csv
```
...
```bash
python "$SKILL_DIR/scripts/analyze.py" \
  --files $UPLOAD_DIR/orders.csv $UPLOAD_DIR/customers.xlsx \
  --action query \
